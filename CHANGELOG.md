...
**CONSTANT Keyword**
- **`CONSTANT`** - Define named constants with `<value> CONSTANT <name>` syntax
  - Extracts value from previous LIT instruction
  - Creates a word that returns the constant value
  - Supports all integer formats (decimal, hexadecimal, negative)
  - Constants are case-insensitive (like all Forth words)
  - Usage examples:
    - `42 CONSTANT ANSWER`
    - `0xFF CONSTANT MAXBYTE`
    - `-1 CONSTANT TRUE`
    - `100 CONSTANT BASE  10 CONSTANT OFFSET  BASE OFFSET +`

**Error Handling**
- New error codes:
  - `ConstantWithoutValue` (-37) - CONSTANT used without preceding literal value
  - `ConstantWithoutName` (-38) - CONSTANT used without name

**Testing**
- Added comprehensive test suite for CONSTANT (`test_constant.cpp`)
...
- CONSTANT implementation extracts value from previous LIT instruction
- Generated bytecode for constants: `LIT <value> ; RET` (6 bytes total)
